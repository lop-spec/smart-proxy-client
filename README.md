...
- 测速自动寻找并验证 Node.js 22+：应用目录、PATH、正在运行的 Pi/Node 及标准安装目录。复用便携 Pi 的运行时，无需修改系统 PATH；找不到时明确报错，不把基础设施故障记为节点失败。代理内核不依赖 Node。
...
- 首页可“移除离线 YAML”：仅移除来源，保留原文件，下次手动启动代理时才应用到内核。不要通过删除原文件间接移除订阅。
