...
(async function () {
  ...
  function populate(mode='ready') {
    ...
    state.mainProcess=mode==='empty'?null:{};state.mainCoreReady=mode!=='empty';
    ...
    if(mode==='pending')state.codexProbePendingKeys.add(entries[0].key);
  ...
  startMainCore=async()=>{__fixture.calls.push({action:'start'});};stopMainCore=async()=>{__fixture.calls.push({action:'stop'});};
