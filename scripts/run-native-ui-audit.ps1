...
'@
 ...
 if(-not [IsolatedNative]::CreateProcessW($exe,$args,[IntPtr]::Zero,[IntPtr]::Zero,$false,0x08000404,$envBlock,$stage,[ref]$si,[ref]$pi)){throw ('CreateProcess failed '+[Runtime.InteropServices.Marshal]::GetLastWin32Error())}
 ...
 if($job -ne [IntPtr]::Zero){[void][IsolatedNative]::TerminateJobObject($job,240);for($i=0;$i -lt 50 -and [IsolatedNative]::ActiveProcesses($job) -gt 0;$i++){Start-Sleep -Milliseconds 100};$result.ownedProcessesRemaining=[IsolatedNative]::ActiveProcesses($job);[void][IsolatedNative]::CloseHandle($job)}
